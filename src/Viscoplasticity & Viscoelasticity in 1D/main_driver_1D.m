...
% inputs %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
loadCase=6;
loadParams= [1000 0.052]; %[0.0026 0.0026];
matMod=5;
totalTime=100;
nTimeSteps=1000;
materialParams=[19600 130 96000 500]; %[0.67 0.67 5]; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% loadcase definition
switch loadCase
    case 3
...
% standard viscoelasticity, convolution
        case 7
            % adapt initial conditions for non zero initial strains
...
%if (matMod == 6 || matMod == 7) 
% outFileName = strcat('viscoelasti_totalTime-',num2str(totalTime), '_nTimesTeps-', num2str(nTimeSteps), '_lc-',num2str(loadCase),'_Model-',num2str(matMod),'.dat');
% outFileID=fopen(outFileName, 'w');
% result = [time' eps' sigma'];
% dlmwrite(outFileName, result, '\t');
%elseif matMod == 5
% outFileName = strcat('viscoplasti_eta-',num2str(materialParams(4)), '_totalTime-',num2str(totalTime), '_lc-',num2str(loadCase),'_nTimesTeps-', num2str(nTimeSteps), '_Model-',num2str(matMod),'.dat');
% outFileID=fopen(outFileName, 'w');
% plasticEps = internalVariables(1,:);
% alphas = internalVariables(2,:);
% result = [time' eps' sigma' plasticEps' alphas'];
% dlmwrite(outFileName, result, '\t');
%end